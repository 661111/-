$().ready(function() {
    //我为什么要单弄个reg.js，而不直接用mian.js?
	//因为极验证，如果我在原来的mian.js里面写进极验证，而不是但弄个reg.js，这样会导致/删除添加播放器无反应
	//reg.js只有注册页面/聊天室引用，所以不对其它有影响
...
var handlerEmbed = function(captchaObj) {
	...
	if (!validate) {
		layer.msg('<font color="#000000">请完成验证</font>');
		e.preventDefault();//对验证码进行保护，防止过期
		return false;
	}
	layer.load();
	$.ajax({
  ...
  $.ajax({
			...
			type: 'post',
			dataType: 'json',
			data:{"name":name,"cloudinfo":cloudinfo,"description":description,"keywords":keywords,"web_zt":web_zt,"gonggao":gonggao},
			success: function(data) {
			var str = data.msg;
			if (data.code == 1) {
  ...
  $.ajax({
	...
	$("[id=search]").click(function() {
		...
		$('#search_list').html('<tr><th><img src="/Public/Other/layer/theme/default/loading-2.gif"  width="20px"/></th></tr>');
		 $.ajax({
			url: 'http://music2.m0x.cn:3000/search',//别打，死了都没音乐搜索接口用
			type: 'GET',
			dataType: 'json',
			data:{
				"keywords":name
  ...
  $.ajax({
			...
			success: function(data) {
			if (data.code == 1) {
			layer.msg('发送消息成功', {icon: 6});
			window.location.href = "?do=chat";
			}else{
...
//验证码加载开始
$('#embed-captcha').html("<p style='text-align: center;'><img src='/Public/Other/layer/theme/xplayer/loading-0.gif' height=25></p>");
$.ajax({
	url: "/Core/Core/web/StartCaptchaServlet.php?t=" + (new Date()).getTime(),
	type: "get",
	dataType: "json",
	success: function(data) {
		console.log(data);
